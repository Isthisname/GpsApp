...
import MapComponent from '../components/maps/MapComponentForTable';

export default function GridComponent({ rows, columns }) {
	const [selectedItems, setSelectedItems] = useState([]);

	const handleSelectionChange = e => {
		const listaPos = rows.filter(row => e?.indexOf(row.id) !== -1);
		// .map(row => ({ lat: row?.location?.latitude, lng: row?.location?.longitude }));

		setSelectedItems(listaPos);
	};
